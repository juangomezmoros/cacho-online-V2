...
export default function GameBoard({
  ...
  const isHumanTurn =
    (!!players && players[currentPlayerIndex]?.id === 0) &&
    (status === GameStatus.InProgress || status === GameStatus.AwaitingDirection);

  useEffect(() => {
    if (status === GameStatus.Reveal && revealState && currentBet) {
      if (animatedCount < revealState.revealedCount) {
        const timer = setTimeout(() => setAnimatedCount((c) => c + 1), 80);
  ...
  // Conteos acumulados seguros por jugador en orden de revelado
  const countsBeforeMap = new Map<number, number>();
  if (status === GameStatus.Reveal && revealOrder && currentBet && players) {
    let cumulative = 0;
    for (const idx of revealOrder) {
      const pl = players[idx];
      if (pl) {
        countsBeforeMap.set(pl.id, cumulative);
        if (!pl.isEliminated) {
          cumulative += countForFace(pl.dice, currentBet.face, currentBet.face === 1);
        }
      }
    }
  }
  ...
  function renderRoundResult() {
    ...
    if (bonusLifePlayer) {
      return (
        <>
          <h2 className="text-lg font-semibold text-yellow-300">¡Vida Extra!</h2>
          <p className="text-xl mt-2 text-cyan-300">
            {bonusLifePlayer?.name ?? 'Jugador'} usó una vida extra y no pierde un dado.
          </p>
        </>
      );
    }
    ...
    if (roundResult.type === RevealType.SalpiconDoubt) {
      const salpText =
        roundResult.actualCount === 5
          ? 'SÍ tenía Salpicón.'
          : `NO tenía Salpicón (tenía ${roundResult.actualCount} caras únicas).`;
      return (
        <>
          <h2 className="text-lg font-semibold text-yellow-300">Resultado de Duda a Salpicón</h2>
          <p className="text-xl mt-2">{betPlayer?.name ?? 'Jugador'} {salpText}</p>
          <p className={`text-xl mt-2 ${roundResult.isSuccess ? 'text-green-400' : 'text-red-400'}`}>
            {actionTaker?.name ?? 'Jugador'} {roundResult.isSuccess ? 'acertó al dudar.' : 'se equivocó al dudar.'}
          </p>
          <p className="text-lg mt-1">
            {players?.find((p) => p.id === roundResult.loserId)?.name ?? 'Jugador'} pierde un dado.
          </p>
        </>
      );
    }
      ...
      return (
        <>
          ...
          <div className="flex items-center justify-center gap-2 text-2xl font-bold">
            Había <span className="text-yellow-400">{roundResult.actualCount}</span>
            <Dice value={betForDisplay!.face} size="sm" />
          </div>
          <p className={`text-xl mt-2 ${roundResult.isSuccess ? 'text-green-400' : 'text-red-400'}`}>
            {actionTaker?.name ?? 'Jugador'} {roundResult.isSuccess ? 'acertó al cazar.' : 'se equivocó al cazar.'}
          </p>
          <p className="text-lg mt-1">
            {roundResult.isSuccess
              ? `${players?.find((p) => p.id === roundResult.gainerId)?.name ?? 'Jugador'} gana un dado o vida.`
              : `${players?.find((p) => p.id === roundResult.loserId)?.name ?? 'Jugador'} pierde un dado.`}
          </p>
        </>
      );
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-6 left-6 bg-green-900/50 p-2 rounded-lg text-center shadow-lg">
        ...
        <div className="text-3xl font-bold text-white">{totalDiceInPlay ?? 0}</div>
      </div>
      ...
      {opponents.map((player, index) => {
        ...
        const isRevealing =
          ...
          playerPositionInRevealOrder !== -1 &&
          playerPositionInRevealOrder <= currentlyRevealingPlayerIndexInRevealOrder;

        const isBeingCounted =
          isRevealing && playerPositionInRevealOrder === currentlyRevealingPlayerIndexInRevealOrder;
      ...
      >
        {(status === GameStatus.InProgress || status === GameStatus.Reveal) && (players?.length ?? 0) > 2 && (
          <div className="absolute top-2 right-2 text-gray-200 text-sm flex items-center gap-1">
            <span>Dirección:</span>
            <span className="font-bold text-lg">{playDirection === 'RIGHT' ? '→' : '←'}</span>
          </div>
        )}

        {/* Durante revelado */}
        {status === GameStatus.Reveal && currentBet && (
          <div className="animate-fade-in">
            <h2 className="text-lg font-semibold text-yellow-300 flex items-center justify-center gap-2">
              {(currentPlayer?.name ?? 'Jugador')} {revealType === RevealType.Doubt ? 'dudó' : 'cazó'}{' '}
              {currentBet.quantity} x <Dice value={currentBet.face} size="sm" />
            </h2>
        ...
        {status === GameStatus.InProgress && currentBet && (
          <>
            {currentBet.isSalpicon ? (
              <div className="animate-fade-in">
                <h2 className="text-2xl font-bold text-orange-400">¡SALPICÓN!</h2>
                <p className="text-lg mt-1">
                  de {players?.find((p) => p.id === currentBet.playerId)?.name ?? 'Jugador'}
                </p>
                {previousBet && (
        ...
        {(status === GameStatus.RoundOver || status === GameStatus.GameOver) && (
          <div className="animate-fade-in">{renderRoundResult()}</div>
        )}

        {status === GameStatus.GameOver && winner && (
          <div className="animate-fade-in mt-4">
            <h2 className="text-2xl font-bold text-yellow-400">¡Fin del Juego!</h2>
            <p className="text-xl">{winner?.name ?? 'Jugador'} ha ganado la partida.</p>
          </div>
        )}

        <p className="text-md text-gray-200 mt-4 h-6">
          {(status === GameStatus.InProgress || status === GameStatus.AwaitingDirection) ? (message ?? '') : ''}
        </p>
      </div>

      {/* Bandeja de dados del humano + panel de apuestas */}
        ...
        {(status === GameStatus.InProgress || status === GameStatus.AwaitingDirection || status === GameStatus.Reveal) ? (
          ...
          <div className="h-[148px] flex items-center justify-center">
            {status === GameStatus.RoundOver && (
              <p className="text-xl font-bold text-gray-300 animate-pulse">Iniciando siguiente ronda...</p>
            )}
