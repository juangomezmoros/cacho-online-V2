...
const GameBoard: React.FC<GameBoardProps> = ({ gameState, onSetDirection, onPlaceBet, onDoubtBet, onSpotOnBet, onSalpicon }) => {
  ...
  const renderRoundResult = () => {
        ...
        return (
            <>
                 ...
                 <p className="text-xl mt-2 text-cyan-300">
                    {bonusLifePlayer.name} usó una vida extra y no pierde un dado.
                 </p>
            </>
        )
    }
    ...
    if (roundResult.type === RevealType.SalpiconDoubt) {
        const salpiconResultText = roundResult.actualCount === 5 ? "SÍ tenía Salpicón." : `NO tenía Salpicón (tenía ${roundResult.actualCount} caras únicas).`;
        return (
            <>
                ...
                <p className={`text-xl mt-2 ${roundResult.isSuccess ? 'text-green-400' : 'text-red-400'}`}>
                    {actionTaker?.name} {roundResult.isSuccess ? 'acertó al dudar.' : 'se equivocó al dudar.'}
                </p>
    ...
    if (roundResult.type === RevealType.SpotOn) {
        return (
            <>
                ...
                <p className={`text-xl mt-2 ${roundResult.isSuccess ? 'text-green-400' : 'text-red-400'}`}>
                    {actionTaker?.name} {roundResult.isSuccess ? 'acertó al cazar.' : 'se equivocó al cazar.'}
                </p>
                <p className="text-lg mt-1">
        ...
        {status === GameStatus.Reveal && currentBet && players[currentPlayerIndex] && (
            <div className="animate-fade-in">
                <h2 className="text-lg font-semibold text-yellow-300 flex items-center justify-center gap-2">
                    {players[currentPlayerIndex].name} {revealType === RevealType.Doubt ? 'dudó' : 'cazó'} {currentBet.quantity} x <Dice value={currentBet.face} size="sm"/>
                </h2>
